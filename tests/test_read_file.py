...
class TestReadFile:
    def test_read_existing_file(self, adapter, temp_file):
        result = adapter.execute({"path": temp_file, "call_id": "call_001"}, {})
        assert result["status"] == "ok"
        assert "line1" in result["output"]
        assert "line5" in result["output"]

    def test_read_with_offset(self, adapter, temp_file):
        result = adapter.execute({"path": temp_file, "offset": 3, "call_id": "call_001"}, {})
        assert result["status"] == "ok"
        assert "line3" in result["output"]

    def test_read_with_limit(self, adapter, temp_file):
        result = adapter.execute({"path": temp_file, "limit": 2, "call_id": "call_001"}, {})
        assert result["status"] == "ok"
        lines = result["output"].strip().split("\n")
        assert len(lines) == 2

    def test_file_not_found(self, adapter):
        result = adapter.execute({"path": "/nonexistent/path.txt", "call_id": "call_001"}, {})
    ...
    def test_missing_path(self, adapter):
        result = adapter.execute({"call_id": "call_001"}, {})
        assert result["status"] == "error"
